...
const authors = computed(() =>
  (vitePressData.frontmatter.value.authors as TeamMember["id"][]).flatMap(
    (id) => {
...
const date = computed(() => {
  ...
  const result = filePath.match(/(?<date>\d{4}-\d{2}-\d{2})-.*$/);
...
onMounted(() => {
  watchEffect(() => {
    if (date.value) {
      datetime.value = new Intl.DateTimeFormat(lang.value, {
        dateStyle: "long",
      }).format(date.value);
    }
  });
...
<template>
  <header class="AppBlogPostHeader">
    ...
    <ul class="authors">
      <li
        v-for="{ avatar, link, name, title } in authors"
        :key="name"
        class="author"
      >
        <img :src="avatar" :alt="name" class="author-avatar" />
        <p class="author-text">
          <a v-if="link" :href="link" target="_blank" class="author-name">{{
            name
          }}</a>
